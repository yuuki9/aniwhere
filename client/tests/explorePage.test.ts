...
test('ExplorePage wires Apps in Toss ads without granting rewards on the client', () => {
  const source = explorePageSource()
  const resultsSheetSource = mapResultsSheetSource()
  ...
  assert.match(source, /className="map-ad-banner-floating"/)
  assert.match(source, /onVisibleChange=\{handleMapBannerVisibleChange\}/)
  assert.match(resultsSheetSource, /<TossBannerAd className="map-results-ad-banner" placement="explore-list-bottom-cta" \/>/)
  assert.match(adSource, /from '@apps-in-toss\/web-bridge'/)
  assert.match(adSource, /if \(didCancel \|\| containerRef\.current == null\) \{[\s\S]*return[\s\S]*\}[\s\S]*setIsCollapsed\(false\)/)
  assert.match(adSource, /onInitializationFailed: \(\) => \{[\s\S]*setIsCollapsed\(true\)/)
  assert.doesNotMatch(adConfigSource, /from '@apps-in-toss\/web-bridge'/)
  ...
  assert.match(adConfigSource, /VITE_TOSS_AD_USE_LIVE_IDS/)
  assert.match(adConfigSource, /private-apps\.tossmini\.com/)
  assert.match(adConfigSource, /runtimeEnvironment === 'sandbox'/)
  assert.doesNotMatch(source, /preloadTossFullScreenAd|maybeShowShopViewInterstitial|showReviewRewardedAd/)
  assert.doesNotMatch(adConfigSource, /loadFullScreenAd|showFullScreenAd|userEarnedReward|VITE_ENABLE_REVIEW_REWARDED_AD/)
  assert.doesNotMatch(adConfigSource, /grantPromotionReward|point|coin|\/rewards|\/points/)
  assert.match(adSource, /debugAdLayout/)
  assert.match(adSource, /aniwhere-debug-ad-layout/)
  assert.match(adSource, /toss-ad-banner-debug/)
  assert.match(adSource, /isTossMockBannerEnabled/)
  assert.match(adSource, /toss-ad-banner-mock/)
  assert.match(adConfigSource, /VITE_TOSS_AD_USE_MOCK_BANNER/)
  assert.match(adConfigSource, /mockAdBanner/)
  assert.match(adConfigSource, /aniwhere-mock-ad-banner/)
  assert.match(styles, /\.toss-ad-banner-slot\s*\{[\s\S]*width:\s*100%;[\s\S]*height:\s*96px;/)
  assert.match(styles, /\.toss-ad-banner-mock\s*\{[\s\S]*width:\s*100%;[\s\S]*height:\s*96px;/)
  assert.match(styles, /\.toss-ad-banner-debug\s*\{[\s\S]*outline:\s*3px solid #ef4444;/)
  assert.match(styles, /\.toss-ad-banner-debug \.toss-ad-banner-slot\s*\{[\s\S]*outline:\s*3px dashed #2563eb;/)
  assert.match(styles, /\.toss-ad-banner-debug \.toss-ad-banner-slot > \*\s*\{[\s\S]*outline:\s*3px dotted #16a34a;/)
  assert.match(styles, /\.map-surface-app-v2\.map-surface-map-ad-visible\s*\{[\s\S]*--map-control-bottom:\s*max\(188px,\s*calc\(env\(safe-area-inset-bottom\) \+ 180px\)\);/)
  assert.match(styles, /\.map-surface-map-ad-visible \.map-list-fab\s*\{[\s\S]*bottom:\s*max\(104px,\s*calc\(env\(safe-area-inset-bottom\) \+ 100px\)\);/)
  assert.match(styles, /\.map-ad-banner-floating\s*\{[\s\S]*position:\s*fixed;[\s\S]*left:\s*50%;[\s\S]*right:\s*auto;[\s\S]*bottom:\s*0;[\s\S]*width:\s*100vw;[\s\S]*transform:\s*translateX\(-50%\);/)
})
...
test('Explore detail back returns to the originating search route when opened from search results', () => {
  ...
  assert.match(source, /const safeStateReturnTo = isSafeExploreReturnTo\(routeState\?\.returnTo\)/)
  assert.match(source, /const safeQueryReturnTo = isInitialListEntry \? isSafeExploreReturnTo\(searchParams\.get\('returnTo'\) \?\? undefined\) : null/)
  assert.match(source, /const safeRouteReturnTo = safeStateReturnTo \?\? safeQueryReturnTo/)
...
test('Explore full list view reserves space around the map toggle button', () => {
  ...
  assert.ok(panelRules.some((rule) => /gap:\s*var\(--ait-space-3\);/.test(rule)))
  assert.ok(panelRules.some((rule) => /padding:\s*var\(--ait-space-4\) 12px 0;/.test(rule)))
  ...
  assert.ok(listButtonRules.some((rule) => /bottom:\s*max\(104px,\s*calc\(env\(safe-area-inset-bottom\) \+ 100px\)\);/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /position:\s*fixed;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /left:\s*50%;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /right:\s*auto;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /width:\s*100vw;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /margin:\s*0;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-results-ad-banner').some((rule) => /transform:\s*translateX\(-50%\);/.test(rule)))
  ...
  assert.ok(listPanelRules.some((rule) => /padding-bottom:\s*max\(52px,\s*calc\(env\(safe-area-inset-bottom\) \+ 48px\)\);/.test(rule)))
})
