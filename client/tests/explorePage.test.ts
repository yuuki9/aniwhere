...
test('ExplorePage extracts the expanded detail info rows into a focused component', () => {
  ...
  assert.ok(cssRuleBodies(styles, '.map-sheet-detail-row-v3 > :has(.map-sheet-detail-icon)').some((rule) => /flex:\s*1 1 auto;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-sheet-detail-row-v3 > :has(.map-sheet-detail-icon) > :has(.map-sheet-detail-icon)').some((rule) => /height:\s*36px;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-sheet-detail-row-v3 > :has(.map-sheet-detail-icon) > :has(.map-sheet-detail-icon)').some((rule) => /flex:\s*0 0 36px;/.test(rule)))
  assert.ok(cssRuleBodies(styles, '.map-sheet-detail-row-v3 > :has(.map-sheet-detail-icon) > :last-child').some((rule) => /flex:\s*1 1 auto;/.test(rule)))
