...
test('Explore detail summary toggles shop favorites through the Swagger favorite endpoints', () => {
  ...
  assert.match(source, /import \{ addFavoriteShop, getShop, getShopFacets, getShops, removeFavoriteShop \} from '\.\.\/shared\/api\/shops'/)
  ...
  assert.match(source, /nextFavorite \? addFavoriteShop\(shopId\) : removeFavoriteShop\(shopId\)/)
  assert.match(source, /setFavoriteShopIds\(\(current\) =>/)
  assert.match(source, /if \(!getStoredAccessToken\(\)\)/)
  ...
  assert.ok(cssRuleBodies(styles, '.map-place-favorite-button').some((rule) => /width:\s*44px;/.test(rule)))
