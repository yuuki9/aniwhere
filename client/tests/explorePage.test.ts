...
test('ExplorePage wires Apps in Toss ads without granting rewards on the client', () => {
  ...
  assert.match(source, /preloadTossFullScreenAd\('interstitial'\)/)
  assert.match(source, /maybeShowShopViewInterstitial\(detailShop\.id\)/)
  assert.match(source, /isReviewRewardedAdPromptEnabled\(\)/)
  assert.match(source, /showReviewRewardedAd\(\)/)
  assert.match(source, /try \{[\s\S]*result = await showReviewRewardedAd\(\)[\s\S]*\} catch \{/)
  assert.match(source, /setReviewRewardAdStatus\('unavailable'\)[\s\S]*setFavoriteToast\('광고를 불러오지 못했어요\. 리뷰는 정상 등록됐어요\.'\)/)
  ...
  assert.match(adSource, /if \(didCancel\) \{[\s\S]*return[\s\S]*\}[\s\S]*setIsCollapsed\(false\)/)
  assert.match(adSource, /onInitializationFailed: \(\) => \{[\s\S]*if \(didCancel\)/)
  assert.match(adConfigSource, /from '@apps-in-toss\/web-bridge'/)
  assert.match(adSource, /TossAds\.attachBanner/)
  assert.match(adConfigSource, /loadFullScreenAd/)
  assert.match(adConfigSource, /showFullScreenAd/)
  assert.match(adConfigSource, /totalViewCount: number/)
  assert.match(adConfigSource, /totalViewCount % SHOP_VIEW_INTERSTITIAL_THRESHOLD/)
  assert.doesNotMatch(adConfigSource, /const viewedCount = viewedShopIds\.length/)
  assert.match(adConfigSource, /const FULL_SCREEN_AD_TIMEOUT_MS = 8_000/)
  assert.match(adConfigSource, /timeoutId = window\.setTimeout\(\(\) => \{[\s\S]*finish\(\{ status: 'ERROR'/)
  assert.match(adConfigSource, /VITE_TOSS_AD_INTERSTITIAL_GROUP_ID/)
  assert.match(adConfigSource, /VITE_TOSS_AD_REWARDED_GROUP_ID/)
  assert.match(adConfigSource, /VITE_ENABLE_REVIEW_REWARDED_AD/)
...
test('Explore detail summary toggles shop favorites through the Swagger favorite endpoints', () => {
  ...
  assert.match(source, /import \{ Button, Modal, Toast \} from '@aniwhere\/tds-mobile'/)
