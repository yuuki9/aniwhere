...
test('ExplorePage extracts the list results sheet into a focused component', () => {
  ...
  assert.match(resultsSheetSource, /getSortedShopImages/)
...
test('ExplorePage extracts the peek bottom sheet into a focused component', () => {
  ...
  assert.match(peekSheetSource, /shop\.averageRating\.toFixed\(1\)/)
  assert.match(peekSheetSource, /shop\.reviewCount/)
...
test('Explore list view is a full search and results surface instead of a map bottom sheet', () => {
  ...
  assert.match(source, /next\.set\('view', 'list'\)[\s\S]*return `\$\{location\.pathname\}\?\$\{next\.toString\(\)\}`/)
...
test('ExplorePage extracts the expanded detail supplement sections into a focused component', () => {
  ...
  assert.match(supplementSource, /to=\{`\/explore\?view=list&keyword=\$\{encodeURIComponent\(work\.name\)\}`\}/)
  assert.doesNotMatch(supplementSource, /view=list&scope=work&keyword/)
...
test('Explore full list view reserves space around the map toggle button', () => {
  ...
  assert.ok(listPanelRules.some((rule) => /padding-right:\s*var\(--ait-space-4\);/.test(rule)))
...
test('Explore list cards use available shop summary fields in a carrot-style dense card', () => {
  ...
  assert.match(resultsSheet, /function getSortedShopImages\(shop: Shop\)/)
  assert.match(resultsSheet, /shop\.averageRating != null/)
  assert.match(resultsSheet, /shop\.averageRating\.toFixed\(1\)/)
  ...
  assert.doesNotMatch(resultsSheet, /listShopReviews/)
  ...
  assert.ok(cssRuleBodies(styles, '.map-results-card-photo').some((rule) => /aspect-ratio:\s*1 \/ 1;/.test(rule)))
