...
test('SearchPage is an input route that sends submitted searches to the explore list', () => {
  ...
  assert.match(source, /navigate\(buildExploreSearchHref\(\{ keyword: trimmed, scope: nextScope, selectedFilters \}\), \{ replace: true \}\)/)
...
test('SearchPage recent searches and nearby CTA remain the pre-search content', () => {
  ...
  assert.match(source, /onClick=\{\(\) => submitSearchToExplore\(item\.keyword, item\.kind \?\? currentSearchScope\)\}/)
...
test('SearchPage shows server autocomplete suggestions for shops and works', () => {
  ...
  assert.match(source, /Promise\.all\([\s\S]*autocompleteScopes\.map\(\(scope\) => getSearchAutocomplete\(\{ q: compactKeyword, scope, limit: 5 \}\)\),[\s\S]*\)/)
  assert.match(source, /items\.map\(\(item\) => \(\{ \.\.\.item, scope \}\)\)/)
  ...
  assert.match(source, /item\.kind === 'WORK' \? '작품' : '매장'/)
})

test('SearchPage sends clicked work suggestions through work search scope', () => {
  const source = searchPageSource()

  assert.match(source, /const submitSearchToExplore = \(nextKeyword: string, nextScope = currentSearchScope, recentKind\?: RecentSearchKind\) => \{/)
  assert.match(source, /navigate\(buildExploreSearchHref\(\{ keyword: trimmed, scope: nextScope, selectedFilters \}\), \{ replace: true \}\)/)
  assert.match(source, /onClick=\{\(\) => submitSearchToExplore\(item\.label, item\.scope, item\.kind === 'WORK' \? 'work' : 'shop'\)\}/)
})
...
test('SearchPage autocomplete panel keeps compact ListRow-like mobile rhythm', () => {
  ...
  const labelRule = cssRuleBody(styles, '.search-autocomplete-item strong')
  const badgeRule = cssRuleBody(styles, '.search-autocomplete-kind')
  ...
  assert.match(badgeRule, /border-radius:\s*var\(--ait-radius-full\);/)
})
