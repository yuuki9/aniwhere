...
test('SearchPage is an input route that sends submitted searches to the explore list', () => {
  ...
  assert.match(source, /navigate\(buildExploreSearchHref\(\{ keyword: trimmed, scope: currentSearchScope, selectedFilters \}\)\)/)
...
test('SearchPage preserves filter editing but leaves selected filter chips to explore results', () => {
  ...
  assert.match(source, /navigate\(buildExploreSearchHref\(\{ keyword, scope: currentSearchScope, selectedFilters: nextFilters \}\)\)/)
