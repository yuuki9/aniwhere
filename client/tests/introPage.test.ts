...
test('IntroPage renders a nickname setup step for new or unnamed Aniwhere users', () => {
  ...
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-panel'), /justify-items:\s*center;/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-options'), /grid-template-columns:\s*repeat\(5, minmax\(0, 1fr\)\);/)
  assert.match(cssRuleBody(styles, ".intro-profile-emoji-option[data-selected='true']"), /border-color:\s*var\(--ait-color-brand\);/)
...
test('IntroPage opens the shared nickname bottom sheet from the ADS mock entry', async () => {
  ...
  try {
    ...
    assert.equal(container.querySelectorAll('.intro-profile-emoji-option').length, 5)
