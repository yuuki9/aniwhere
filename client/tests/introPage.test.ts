...
test('IntroPage renders a nickname setup step for new or unnamed Aniwhere users', () => {
  ...
  assert.match(source, /import \{ Asset, Button, Modal, TextField, Top \} from '@aniwhere\/tds-mobile'/)
  assert.match(source, /<Modal open=\{open\}>/)
  ...
  assert.match(source, /저장 전에 닉네임 중복 여부를 확인해요\./)
  assert.match(source, /닉네임 저장하기/)
  assert.match(source, /환영합니다/)
  assert.match(source, /Top\.RightAssetContent/)
  assert.match(source, /Asset\.Lottie/)
  assert.match(source, /confetti-spot\.json/)
  ...
  assert.match(cssRuleBody(styles, '.ait-modal-overlay'), /position:\s*fixed;/)
  assert.match(cssRuleBody(styles, '.ait-modal-content'), /width:\s*min\(342px, calc\(100vw - 32px\)\);/)
