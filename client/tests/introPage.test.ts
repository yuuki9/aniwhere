...
test('IntroPage renders a nickname setup step for new or unnamed Aniwhere users', () => {
  ...
  assert.match(source, /createRandomProfileEmojiSet\(PROFILE_EMOJI_INTRO_COUNT\)/)
  ...
  assert.match(source, /PROFILE_EMOJI_INTRO_COUNT/)
  assert.match(source, /import \{ Asset, BottomSheet, Button, TextField \} from '@aniwhere\/tds-mobile'/)
  assert.match(source, /PROFILE_EMOJI_OPTIONS/)
  assert.doesNotMatch(source, /DEFAULT_PROFILE_EMOJI_ID/)
  assert.match(source, /PROFILE_EMOJI_RAIL_FRAME/)
  assert.match(source, /openNicknameOnboardingSheet/)
  assert.match(source, /setProfileEmojiSet\(emojiSet\)/)
  ...
  assert.match(source, /className="intro-nickname-sheet-cta"/)
  ...
  assert.match(source, /style=\{\{ '--intro-profile-emoji-tone': option\.tone \} as CSSProperties\}/)
  assert.match(source, /scrollIntoView\?\.\(\{ block: 'nearest', inline: 'center' \}\)/)
  assert.match(source, /ref=\{isSelected \? selectedEmojiButtonRef : undefined\}/)
  ...
  assert.doesNotMatch(source, /form="intro-nickname-form"/)
  ...
  assert.match(cssRuleBody(styles, '.intro-nickname-sheet-cta'), /padding:\s*var\(--ait-space-1\) var\(--ait-space-8\) var\(--ait-space-8\);/)
  ...
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-panel'), /padding-inline:\s*var\(--ait-space-2\);/)
  ...
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-options'), /min-height:\s*76px;/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-options'), /justify-content:\s*flex-start;/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-options'), /padding:\s*var\(--ait-space-1\) calc\(50% - 38px\);/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-options'), /scroll-padding-inline:\s*calc\(50% - 38px\);/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-option'), /flex:\s*0 0 46px;/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-option'), /height:\s*46px;/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-option'), /background:\s*color-mix\(in srgb, var\(--intro-profile-emoji-tone\) 42%, var\(--ait-color-gray-0\)\);/)
  assert.match(cssRuleBody(styles, '.intro-profile-emoji-image'), /background:\s*transparent;/)
  assert.match(cssRuleBody(styles, ".intro-profile-emoji-option[data-selected='true']"), /flex-basis:\s*76px;/)
  assert.match(cssRuleBody(styles, ".intro-profile-emoji-option[data-selected='true']"), /height:\s*76px;/)
  assert.match(cssRuleBody(styles, ".intro-profile-emoji-option[data-selected='true']"), /background:\s*var\(--intro-profile-emoji-tone\);/)
...
test('IntroPage opens the shared nickname bottom sheet from the ADS mock entry', async () => {
  ...
  try {
    ...
    assert.equal(container.querySelectorAll('.intro-profile-emoji-option').length, 10)
