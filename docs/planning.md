...
- 관리자 페이지 MVP에는 샵 수동 등록 기능이 포함되어야 한다.
- 후기/제보는 사용자가 작성하고, 포인트는 관리자가 검수 후 지급한다.
- 점주/사장 기능은 아직 제공하지 않는다.
- 관리자 또는 권한 있는 내부 유저만 admin 페이지에서 매장을 등록한다.
- 점주 제휴형 기능은 사용자 규모 확보 후 재검토한다.
