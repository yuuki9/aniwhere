...
| Nickname emoji | Follow-up API decision | The current Swagger/user profile contract stores only `nickname`; no emoji/avatar field is available for Aniwhere profile persistence yet. The frontend now lets users choose one of five Toss static emoji assets and carries that choice in route state for the welcome toast only. When the backend adds a stable profile emoji field, wire this selected `id` or server-agreed key into the nickname/profile save payload instead of embedding emoji into the nickname string. |
