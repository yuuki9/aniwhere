...
| Nickname emoji | Follow-up API decision / Product-approved | The current Swagger/user profile contract stores only `nickname`; no emoji/avatar field is available for Aniwhere profile persistence yet. The frontend now lets users choose one curated Toss static emoji asset from a 9-item horizontal rail, with the selected item expanded like the Apps in Toss game profile setup reference. The selected emoji is carried in route state for the welcome toast only. When the backend adds a stable profile emoji field, wire this selected `id` or server-agreed key into the nickname/profile save payload instead of embedding emoji into the nickname string. |
