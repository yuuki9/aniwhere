# Git 컨벤션

브랜치 · 커밋 · 태그 규칙을 통일해 리뷰와 릴리스를 단순하게 만듭니다.  
현재 커밋 로그에서 쓰는 `type(scope): 설명` 형식을 기준으로 정리했습니다.

## 브랜치 전략

| 브랜치 | 목적 | 설명 |
|--------|------|------|
| `main` | 운영(또는 배포 기준) 최신 | 항상 배포 가능하거나 방금 배포된 코드를 가리키도록 유지 |
| `develop` | 개발 통합 | 기능이 모이는 기본 개발 브랜치(팀에서 도입 시) |
| `release/*` | 릴리스·스테이징 | QA·스테이징 배포용. 예: `release/v1.2.0` |
| `feature/*` | 기능 개발 | 예: `feature/login-api`, `feature/store-search` |

### 네이밍 예시

- 기능: `feature/community-post-api`, `feature/map-store-filter`
- 릴리스: `release/v1.2.0`, `release/hotfix-payment`

자동화나 보조 작업이 있더라도, 기본 작업 브랜치는 `feature/*`, `fix/*`, `chore/*`로 통일합니다.
Aniwhere 저장소에서는 AI 도구가 만든 브랜치라도 `codex/*` 같은 도구명 prefix를 사용하지 않습니다.

## 커밋 메시지

### 언어

- **커밋 로그(제목·본문)는 반드시 한글로 작성합니다.** PR·MR 설명도 동일 원칙을 따르는 것을 권장합니다.
- `type`, `scope` 같은 접두 형식은 아래 형식을 유지하고, 콜론 뒤 설명은 한글로 씁니다.
- Codex가 `git commit`, `git push`, PR handoff를 수행할 때도 같은 규칙을 반드시 적용합니다.
- 커밋 전 메시지의 콜론 뒤 설명이 영어이면 커밋하지 말고 한글 메시지로 다시 작성합니다.
- 외부 도구 예시나 Conventional Commit의 `type(scope):` 접두어는 영어를 허용하지만, 변경 설명은 한글이어야 합니다.

### 형식

```
type(scope): 짧은 설명 (한 줄, 명령형 또는 현재형)
```

- **scope**: 변경이 큰 영역. 모노레포 기준으로 `client`, `server`, `docs` 등.
- 본문이 필요하면 한 줄 띄우고 `Why`, `What` 순으로 보강.

### 실제 로그 예시

- `feat(client): 토스 쇼핑·커뮤니티 웹 프론트 빌드 추가`
- `chore(server): gradle.properties로 버전 1.0.0 관리`

### type

| 타입 | 의미 |
|------|------|
| `feat` | 새 기능 |
| `fix` | 버그 수정 |
| `docs` | 문서만 |
| `style` | 포맷·세미콜론·공백 등 (동작 변화 없음) |
| `refactor` | 리팩터링 (동작 유지) |
| `test` | 테스트 추가·수정 |
| `chore` | 빌드·설정·잡무 |

## 권장 워크플로우

1. `develop`(또는 합의된 기본 브랜치)에서 `feature/...` 생성  
2. 컨벤션에 맞게 커밋  
3. PR(Merge Request)로 리뷰  
4. `develop`에 머지 → 필요 시 `release/...`에서 스테이징  
5. 검증 후 `main`에 반영(머지 또는 패치)

## PR 작업 단위

- PR은 하나의 리뷰 가능한 관심사를 기준으로 나눕니다.
- 기능 변경, 문서 정리, 리팩터링, 후속 UX 작업은 가능하면 별도 브랜치와 PR로 분리합니다.
- PR 설명은 항상 `.github/PULL_REQUEST_TEMPLATE.md`를 먼저 읽고 그 구조를 유지해 작성합니다.
- PR 설명에는 실제 변경 범위, 검증 결과, 머지 후 확인할 사항만 기록합니다.
- 임의의 줄 수 기준은 PR 생성 조건이나 설명에 적지 않습니다.
- 최종 merge 권한은 서버관리자에게 있다고 보고, Codex는 브랜치 push와 PR 생성/핸드오프까지만 책임집니다.
- PR 생성 단계의 필수 산출물은 `PR 생성 URL`, `PR title`, `.github/PULL_REQUEST_TEMPLATE.md` 구조를 그대로 따른 copy/paste 가능한 PR description입니다.
- 사용자는 GitHub 웹 로그인 흐름으로 PR을 만드는 경우가 많습니다. GitHub connector나 `gh` 인증 문제로 PR을 직접 만들 수 없더라도 URL만 전달하지 말고, PR title과 `.github/PULL_REQUEST_TEMPLATE.md` 구조를 따른 PR description을 항상 복사/붙여넣기 가능한 형태로 함께 제공합니다.
- PR description을 사용자가 그대로 전체 복사할 수 있게 작성합니다. 특별한 요청이 없으면 설명 전체를 fenced code block(``` … ```)으로 감싸지 말고, 본문 안에도 불필요한 triple backtick을 넣지 않습니다.

## 버전 태그 (`v*`)

- **릴리스 지점**에만 `v1.0.0`처럼 태그를 둡니다. 매 커밋마다 태그할 필요는 없습니다.
- 한 번 올린 태그는 **개발이 이어진다고 해서 삭제할 필요 없습니다.** 이미 clone·CI가 참조했을 수 있습니다.
- 잘못 찍은 태그만 정리할 때는 원격·로컬 모두 조정과 팀 공지가 필요합니다.

## 참고

- GitHub Pull Request / GitLab Merge Request 템플릿 활용 권장  
- 커밋은 “무엇을·왜” 한 줄에 드러나게 작성(위 언어 규칙 준수)
